...
export class Tab1Page {
  ...
  */
  ...
  constructor(private leerArticulosFicheroHttp: HttpClient, public gestionNoticiasLeerService: GestionNoticiasLeerService) {
   //LLamamos al metodo de consulta commun
   //le pasamos la direccion de la consulta de la categoria bussines
    this.consultaGet("https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=9d1de9ce98954c24a98751fc8fbea520"); 
  }
  ...
  public consultaGet(categoria :string) {
    ...
    //le pasamos categoria como parametro
    let respuesta: Observable<RespuestaNoticias> = this.leerArticulosFicheroHttp.get<RespuestaNoticias>(categoria);
  ...
  //Metodo para borrar las lista de noticias
  public borrarNoticia(){
    for(let i = 0 ; i < this.listaNoticias.length ; i++ ){
      this.listaNoticias.splice(i);
    }
}
}
